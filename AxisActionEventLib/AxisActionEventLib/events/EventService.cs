        ...
        private GetEventInstancesResponse parseGetEventInstancesResponse(ServiceResponse Response)
        {
            ...
            if (response.IsSuccess)
            {
                try
                {
                    ...
                    {
                        getTopics("tns1:" + el.Name.LocalName + "/tnsaxis:", EventTriggers, el);
                    }
                    //Post
        ...
        private void getTopics(String Topic, List<EventTrigger> Triggers, XElement Element)
        {
            try
            {
                ...
                foreach (XElement el in Element.Elements())
                {
                    ...
                    {
                        getTopics(Topic + el.Name.LocalName + "/", Triggers, el);
                    }
                    else
                    {
                        //create new eventTrigger with Topic & IsProperty attribute
                        newEventTrigger = new EventTrigger(Topic + el.Name.LocalName, el.Element(NS_EVENT + "MessageInstance").HasAttribute(NS_EVENT + "isProperty"));
